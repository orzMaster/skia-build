...
def main():
  ...
  try:
    resp = urllib.request.urlopen(urllib.request.Request('https://api.github.com/repos/532479301/skia-build/releases/tags/' + version, headers=headers)).read()
  except urllib.error.URLError as e:
    data = '{"tag_name":"' + version + '","name":"' + version + '"}'
    resp = urllib.request.urlopen(urllib.request.Request('https://api.github.com/repos/532479301/skia-build/releases', data=data.encode('utf-8'), headers=headers)).read()
